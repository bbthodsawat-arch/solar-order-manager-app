...
export interface DbHealthStatus {
  ...
  // Kept as an unconfigured compatibility field for existing UI types. It is
  // never probed, selected, or written to by the runtime.
...
let autoFailover = false; // No cross-database failover.
...
function notifySubscribers() {
  const state = { preferredProvider, actualProvider, autoFailover, health: healthStatus };
  subscribers.forEach((cb) => {
    try { cb(state); } catch (err) { console.error('Database subscriber error:', err); }
  });
}

function recalculateActualProvider() {
  const isOnline = typeof navigator !== 'undefined' ? navigator.onLine : true;
  actualProvider = isOnline && healthStatus.firebase.status !== 'error' ? 'firebase' : 'local';
}

function parseQueue(): any[] {
  try {
    return JSON.parse(localStorage.getItem('offline_transactions_queue') || '[]');
  } catch {
    return [];
  }
}
...
export const dbManager = {
  ...
  setPreferredProvider(provider: DbProvider) {
    // Supabase can no longer be selected. Local is only an offline mode;
    // Firebase remains the canonical cloud source of truth.
    preferredProvider = provider === 'local' ? 'local' : 'firebase';
    localStorage.setItem('solar_preferred_database_mode', preferredProvider);
    recalculateActualProvider();
    notifySubscribers();
  },
  setAutoFailover(_enabled: boolean) {
    autoFailover = false;
    localStorage.setItem('solar_auto_failover_enabled', 'false');
    recalculateActualProvider();
    notifySubscribers();
  },
  subscribe(callback: SubscriberCallback) {
    subscribers.add(callback);
    callback({ preferredProvider, actualProvider, autoFailover, health: healthStatus });
  ...
  async runDiagnostics(): Promise<DbHealthStatus> {
    ...
    healthStatus.local = {
      status: 'healthy',
      message: `Offline queue พร้อมใช้งาน (${parseQueue().length} รายการ)`,
    };

    if (!isOnline) {
      healthStatus.firebase = { status: 'offline', latencyMs: 0, message: 'อุปกรณ์ออฟไลน์ — ใช้ Firebase SDK offline cache/queue' };
  ...
  getErrorLogs(): DbSyncError[] {
    try { return JSON.parse(localStorage.getItem('solar_db_sync_errors') || '[]'); } catch { return []; }
  },
  clearErrorLogs() {
  ...
  addErrorLog(source: DbSyncError['source'], errorType: string, errorMessage: string, autoFailoverTriggered: boolean) {
    try {
      ...
      const newLog: DbSyncError = {
        ...
        timestamp: new Date().toISOString(), source, errorType, errorMessage, autoFailoverTriggered,
      };
