...
class QueryBuilder {
  ...
  private async read():Promise<{data:Row[];error:any}>{try{const constraints:any[]=[];for(const f of this.filters)constraints.push(where(f.field,'==',f.value));if(this.orderField)constraints.push(orderBy(this.orderField,this.orderDirection));if(this.limitCount)constraints.push(limit(this.limitCount));const snapshot=await getDocs(query(collection(db,this.table),...constraints));const data:Row[]=snapshot.docs.map(item=>({id:item.id,...(item.data() as Row)}));return {data:this.selected?data.map(row=>{const result:Row={id:row.id};for(const key of this.selected!.split(',').map(k=>k.trim()))if(key)result[key]=row[key];return result;}):data,error:null};}catch(error){return {data:[],error};}}
}
