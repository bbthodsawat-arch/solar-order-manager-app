  ...
  private async execute(single: boolean) {
    try {
      ...
      if (this.mode === 'insert') {
        ...
        for (const row of rows) { const id = row?.id ? String(row.id) : undefined; if (id) await setDoc(doc(db, this.table, id), row, { merge: true }); else await addDoc(ref, row); }
      ...
      if (this.mode === 'update' || this.mode === 'delete') {
        for (const row of rows) { const target = doc(db, this.table, row.id); if (this.mode === 'update') await updateDoc(target, this.payload); else await deleteDoc(target); }
...
export async function createNewUserWithPassword(email: string, password: string): Promise<string> { const result = await firebaseCreateNewUserWithPassword(email, password); if (result.error || !result.user) throw result.error || new Error('Firebase user creation failed'); return result.user.uid; }
