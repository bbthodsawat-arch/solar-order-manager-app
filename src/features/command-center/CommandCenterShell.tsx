...
import { COMMAND_REGISTRY, getRegistryHealth, type CommandDefinition } from './registry';
...
export function CommandCenterShell({ children, onSelectCommand, additionalCommands = [] }: CommandCenterShellProps) {
  ...
  const registryHealth = useMemo(() => getRegistryHealth([...COMMAND_REGISTRY, ...additionalCommands]), [additionalCommands]);
  ...
  useEffect(() => {
    if (selectedCommandId && !available.some(command => command.id === selectedCommandId)) setSelectedCommandId(null);
  ...
  return <div className="max-w-[1600px] mx-auto space-y-5 pb-12">
    <CommandCenterOverview user={appUser} commands={available} registryHealth={registryHealth} onSelectCommand={selectCommand}/>
    <section className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-7">
      ...
      {visibleDomains.length > 0 ? <div className="mt-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{visibleDomains.map(domain => <div key={domain.id} className="rounded-2xl border border-slate-100 p-3 dark:border-slate-800"><div className="text-[10px] font-black uppercase tracking-wider text-slate-400">{domain.title}</div><div className="mt-2 space-y-1">{domain.commands.map(command => <button key={command.id} type="button" onClick={() => selectCommand(command)} aria-pressed={selectedCommand?.id === command.id} className={`flex w-full items-start justify-between gap-3 rounded-xl px-2.5 py-2 text-left transition hover:bg-slate-50 dark:hover:bg-slate-800 ${selectedCommand?.id === command.id ? 'bg-brand-soft ring-1 ring-brand/20' : ''} ${command.danger ? 'text-rose-600' : ''}`}><span><span className="block text-xs font-black">{command.title}</span><span className="block text-[10px] text-slate-400">{command.description}</span></span>{command.danger && <ShieldAlert size={15}/>}</button>)}</div></div>)}</div> : <div role="status" className="mt-5 rounded-2xl border border-dashed border-slate-200 p-8 text-center dark:border-slate-700"><div className="text-sm font-black">ไม่มีคำสั่งที่ได้รับอนุญาต</div><div className="mt-1 text-xs font-medium text-slate-400">Registry ยังทำงานอยู่ แต่ permission policy ไม่อนุญาตให้บัญชีนี้เปิดคำสั่งใดในขณะนี้</div></div>}
      {query.trim() && visible.length === 0 && available.length > 0 && <div role="status" className="mt-5 rounded-2xl border border-dashed border-slate-200 p-6 text-center text-xs font-bold text-slate-400 dark:border-slate-700">ไม่พบคำสั่งที่ตรงกับ “{query}”</div>}
    </section>
