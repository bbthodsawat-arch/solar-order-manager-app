...
export default function App() {
 ...
 const handleQuickAdd=(type:TransactionType,category:TransactionCategory,detail?:string,amount?:number)=>{if(!userPerms.canAddTransactions)return;setQuickAddData({type,category,detail,amount});setActiveTab('pos');};
 ...
 return <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors flex flex-col font-sans">
  ...
  <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-5 sm:py-6 pb-28 lg:pb-8">
   <AnimatePresence mode="wait"><motion.div key={activeTab} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}} transition={{duration:.14}}>
    ...
    {activeTab==='history'&&<History/>}{activeTab==='reports'&&userPerms.canViewReports&&<Reports/>}{activeTab==='customers'&&<CustomerCRM/>}{activeTab==='installations'&&<InstallationWarrantyManager/>}{activeTab==='users'&&<UserManagement/>}{activeTab==='audit_logs'&&(userPerms.canViewAuditLogs||isAdminOrOwner)&&<AuditLogManager/>}{activeTab==='settings'&&<CommandCenter onNavigateToUsers={()=>setActiveTab('users')} onNavigateToAudit={()=>setActiveTab('audit_logs')} onLockApp={()=>setIsUnlocked(false)}/>} 
