...
import { collection, query, orderBy, onSnapshot, addDoc, doc, deleteDoc, updateDoc, deleteField, getDocs, writeBatch } from 'firebase/firestore';
...
function removeUndefinedFields<T>(obj: T): T {
  ...
  for (const [key, value] of Object.entries(obj)) if (value !== undefined) cleaned[key] = typeof value === 'object' && value !== null ? removeUndefinedFields(value) : value;
  return cleaned as T;
}
...
export function useCustomers() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setLoading(false);
    }, (error) => { console.error('Error fetching customers:', error); setLoading(false); });
    return () => unsubscribe();
  }, [user]);
  ...
  const addCustomer = async (customer: Omit<Customer, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>): Promise<string> => {
    ...
    try {
      ...
      const cleanedData = removeUndefinedFields({ ...customer, createdAt: now, updatedAt: now, createdBy: user.uid });
      return (await addDoc(collection(db, 'customers'), cleanedData)).id;
  ...
  const updateCustomer = async (id: string, updates: Partial<Customer>): Promise<void> => {
    try { await updateDoc(doc(db, 'customers', id), prepareUpdateData({ ...updates, updatedAt: new Date().toISOString() })); }
    catch (error) { console.error('Error updating customer:', error); throw error; }
  };

  const deleteCustomer = async (id: string): Promise<void> => {
    try { await deleteDoc(doc(db, 'customers', id)); }
    catch (error) { console.error('Error deleting customer:', error); throw error; }
  };
  ...
  const findOrCreateCustomer = async (customerData: Partial<Customer>): Promise<string> => {
    ...
    const existing = customers.find(c => (taxIdToMatch && c.customerTaxId?.trim() === taxIdToMatch) || (phoneToMatch && c.phoneNumber?.trim() === phoneToMatch) || c.name.trim().toLowerCase() === nameToMatch);
    if (existing?.id) {
    ...
    return addCustomer({ name: customerData.name.trim(), phoneNumber: customerData.phoneNumber || '', email: customerData.email || '', customerTaxId: customerData.customerTaxId || '', customerBranch: customerData.customerBranch || 'สำนักงานใหญ่', customerAddress: customerData.customerAddress || '', district: customerData.district || '', province: customerData.province || 'กรุงเทพมหานคร', zipcode: customerData.zipcode || '', note: customerData.note || '' });
  };

  /** Delete every customer in Firestore using safe batches below the 500-operation limit. */
  const deleteAllCustomers = async (): Promise<void> => {
    try {
      const snapshot = await getDocs(collection(db, 'customers'));
      for (let start = 0; start < snapshot.docs.length; start += 450) {
        const batch = writeBatch(db);
        snapshot.docs.slice(start, start + 450).forEach((customer) => batch.delete(customer.ref));
        await batch.commit();
      }
      const verify = await getDocs(collection(db, 'customers'));
      if (!verify.empty) throw new Error(`Customer reset verification failed: ${verify.size} documents remain`);
    } catch (error) { console.error('Error deleting all customers:', error); throw error; }
  };

  return { customers, loading, addCustomer, updateCustomer, deleteCustomer, findOrCreateCustomer, deleteAllCustomers };
}
