  ...
  return (
    <>
      <button
        ...
        className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl border text-xs font-bold transition-all shadow-2xs cursor-pointer active:scale-95 ${
          isActuallySyncing
            ? 'animate-pulse ring-2 ring-indigo-500/35 border-indigo-500/50 bg-indigo-500/15 text-indigo-800 dark:text-indigo-200'
            : isOffline
        ...
        title="ดูสถานะ Firebase และการซิงค์ข้อมูล"
      >
        {isOffline ? <HardDrive size={14} /> : <Cloud size={14} />}
        <span className="hidden sm:inline text-[11px] font-black">
          {isActuallySyncing ? 'DB: Firebase (กำลังซิงค์...)' : isOffline ? 'DB: Local (ออฟไลน์)' : 'DB: Firebase'}
        </span>
      </button>
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                    <ShieldCheck size={13} /> Firebase / Firestore
                  </div>
                  <h3 className="mt-2 text-lg font-black text-slate-900 dark:text-white">
                    สถานะข้อมูลและการซิงค์
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Firebase/Firestore เป็นฐานข้อมูล Cloud หลักของระบบ
                  </p>
                </div>
                <button
                  ...
                  className="p-2 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 cursor-pointer"
                  aria-label="ปิด"
                >
                  <X size={18} />
                </button>
              ...
              <div className={`p-4 rounded-2xl border flex items-center gap-3 ${
                ...
                <div className="w-11 h-11 rounded-2xl bg-emerald-500 text-white flex items-center justify-center shrink-0">
                  {status === 'error' ? <AlertTriangle size={22} /> : isOffline ? <CloudOff size={22} /> : <ShieldCheck size={22} />}
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-black text-slate-900 dark:text-white">
                    {status === 'error' ? 'การเชื่อมต่อมีปัญหา' : isOffline ? 'โหมดออฟไลน์' : 'Firebase เชื่อมต่อพร้อมใช้งาน'}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    ซิงค์ล่าสุด {relativeSyncTime}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                <div>
                  <p className="text-xs font-black text-slate-800 dark:text-white">รายการค้างส่ง</p>
                  <p className="text-[11px] text-slate-500 mt-1">
                    {pendingCount > 0 ? `${pendingCount} รายการรอซิงค์` : '0 รายการ (สมบูรณ์)'}
                  </p>
                </div>
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 disabled:text-slate-500 text-white font-black text-xs flex items-center gap-2 cursor-pointer disabled:cursor-not-allowed"
                >
                  <RefreshCw size={14} className={isActuallySyncing ? 'animate-spin' : ''} />
                  {isActuallySyncing ? 'กำลังซิงค์...' : 'Sync Now'}
                </button>
              </div>
            </motion.div>
          </div>
